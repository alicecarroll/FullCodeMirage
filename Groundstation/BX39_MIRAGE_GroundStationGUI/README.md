...
Then open `http://127.0.0.1:8080`. The gateway serves the GUI, listens for the main MCU's TCP status stream on port `5001`, decodes the packed 217-byte `MainSystemStatusPacket`, streams telemetry to the browser, and sends GUI/terminal commands back over the active payload TCP connection. A command is acknowledged only after a newer payload status packet reports that a recognized command was received.
