...
void heater_set_mode(HeaterSystem* system, uint8_t heater_id, HeaterControlMode mode)
{
    ...
    if (mode != HEATER_MODE_BANGBANG && mode != HEATER_MODE_PID && mode != HEATER_MODE_MANUAL) {
        ESP_LOGW(TAG, "Invalid control mode: %d", mode);
        return;
    }
...
bool heater_send_config_to_thermal(uint8_t heater_id, int16_t current_temp)
{
    ...
    // The thermal MCU has no per-channel enable flag. Manual 0% is its
    // supported per-channel off representation.
    uint8_t mode_to_send = !config->enabled
                          ? static_cast<uint8_t>(HEATER_MODE_MANUAL)
                          : (config->mode == HEATER_MODE_MANUAL)
